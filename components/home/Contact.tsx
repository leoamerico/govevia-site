    ...
    <section className="py-24 bg-deep-navy" id="contato">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
          {/* COLUNA ESQUERDA — caixa unificada: Implantação consultiva + Responsável */}
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            <div className="bg-white/5 p-8 rounded-lg border border-white/10 text-white space-y-6">
              {/* Bloco notice */}
              {(notice.title.trim().length > 0 || notice.body.trim().length > 0) ? (
                <div>
                  {notice.title.trim().length > 0 ? (
                    <h3 className="font-serif font-semibold text-xl text-white mb-3">{notice.title}</h3>
                  ) : null}
                  {notice.body.trim().length > 0 ? (
              ...
              {(ceo.label.trim().length > 0 || ceo.name.trim().length > 0) ? (
                <div>
                  ...
                  {ceo.name.trim().length > 0 ? (
                    <p className="text-xl font-serif font-bold mb-2">{ceo.name}</p>
                  ) : null}
                  {ceo.role ? (
                    <p className="text-xs uppercase tracking-wider text-primary-light font-semibold mb-3">{ceo.role}</p>
                  ) : null}
                  {ceo.email ? (
                    <a href={`mailto:${ceo.email}`} className="block text-sm text-blue-400 hover:text-blue-300 transition-colors mb-3">
                      {ceo.email}
                    </a>
                  ) : null}
                  {ceo.whatsappUrl && ceo.phone ? (
                    <a href={ceo.whatsappUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-slate-200 hover:text-white transition-colors">
