      ...
      <main>
        ...
        <Defensibility
          ...
          trail={{
            ...
            items: [
              { label: '1. Chegada', value: 'Tudo começa aqui', body: 'Assim que seu documento ou pedido chega, registramos o dia, hora e quem enviou — como um carimbo oficial que ninguém pode apagar.', technical: 'Criação automática de registro imutável com timestamp UTC assinado digitalmente, hash SHA-256 do documento original, metadados de origem (IP, canal, usuário) e vinculação ao requerente via CPF/CNPJ ou login Gov.br. Garante cadeia de custódia desde o primeiro instante (LGPD + LAI).' },
              { label: '2. Verificação', value: 'Quem pode cuidar disso?', body: 'Conferimos se a pessoa ou setor certo recebeu o pedido e se está dentro do prazo. É como verificar se o ticket foi para a fila correta.', technical: 'Motor de regras executa verificação automática de competência (unidade/cargo), alçada decisória, impedimentos legais (suspeição, nepotismo) e prazos decadenciais/prescricionais antes da distribuição.' },
              { label: '3. Explicação', value: 'Por que vamos decidir assim?', body: 'Todo servidor deve explicar com clareza qual lei ou regra está usando. É a parte onde ele justifica a decisão para você e para quem for fiscalizar depois.', technical: 'Fundamentação jurídica obrigatória registrada em campos estruturados (artigos de lei, jurisprudência, pareceres técnicos) para transparência ativa (LAI) e auditoria futura.' },
              { label: '4. Decisão', value: 'A palavra final', body: 'Só o servidor responsável pode dar a resposta definitiva. Ele assina e assume a decisão — sempre uma pessoa de verdade, nunca uma máquina sozinha.', technical: 'Ato administrativo praticado exclusivamente por agente público identificado e autenticado (certificado ICP-Brasil ou Gov.br), com assinatura eletrônica qualificada e registro de intenção clara no sistema.' },
              { label: '5. Prova imutável', value: 'Tudo guardado para sempre', body: 'Cada passo recebe um selo digital (um código único). Se alguém tentar mudar qualquer coisa depois, o sistema avisa imediatamente.', technical: 'Geração de hash encadeado (Merkle Tree) em cada etapa. Toda a cadeia é verificável por auditores externos via API pública ou exportação de prova criptográfica completa (RFC 3161).' },
            ],
          }}
