...
// Gate D-04 — WCAG contrast validation based on explicit pairs in tokens.json
...
for (const pair of pairs) {
  ...
  const ratio = contrastRatio(hexToRgb(fgNode.hex), hexToRgb(bgNode.hex))
  if (ratio + 1e-6 < pair.ratio_min) {
    failures++
    console.error(`FALHA: ${pair.level ?? 'WCAG'} ${pair.fg} on ${pair.bg} → ${ratio.toFixed(2)} (min ${pair.ratio_min})`)
  }
}
